...
var log_default = (message, status = "default") => {
  if (status === "default")
    console.log("\n ", import_chalk5.default.hex("#000").bgGray(message));
  if (status === "info")
    console.log(import_chalk5.default.bgHex("#6366f1").white(" Info "), message);
  if (status === "success")
    console.log("\n ", import_chalk5.default.hex("#8EEDB3")(message));
  if (status === "important")
    console.log("\n ", import_chalk5.default.bgYellow.black(message));
  if (status === "error")
    console.error("\n ", import_chalk5.default.red(message));
};

// src/utilities/loader.ts
var import_ora = __toESM(require("ora"));
var loader_default = (text) => (0, import_ora.default)({ text, spinner: "dots2", discardStdin: false, color: "cyan" });
...
var cloneRepos = (Repos, Dir) => new Promise((fulfill, reject) => {
  ...
  import_shelljs.default.echo(`Error: ${err.message}`);
...
var colorize_default = (message, status = "default") => {
  if (status === "default")
    return import_chalk6.default.hex("#000").bgGray(message);
  if (status === "info")
    return `${import_chalk6.default.bgHex("#6366f1").white(" Info ")} ${message}`;
  if (status === "success")
    return import_chalk6.default.hex("#8EEDB3").bold(message);
  if (status === "important")
    return import_chalk6.default.bgYellow.black(message);
  if (status === "error")
    return import_chalk6.default.red(message);
  return "";
};
...
async function bootUpSave() {
  ...
  try {
    ...
    if (!import_fs.default.existsSync(folderName)) {
    ...
    console.error(import_chalk7.default.red(JSON.stringify(error)));
  }
}
