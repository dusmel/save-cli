...
var require_figures = __commonJS({
  "node_modules/figures/index.js"(exports, module) {
    ...
    module.exports = Object.assign(fn, figures3);
...
var Separator = class {
  separator = chalk2.dim(new Array(15).join(import_figures.default.line));
  type = "separator";
...
function useKeypress(userHandler) {
  ...
  useEffect((rl) => {
    const handler = AsyncResource.bind(context.mergeStateUpdates((_input, event) => {
...
function usePagination(output, { active, pageSize = 7 }) {
  ...
  return section + "\n" + chalk3.dim("(Move up and down to reveal more choices)");
}
...
var esm_default = createPrompt((config, done) => {
  ...
  let message = chalk4.bold(config.message);
  if (firstRender.current) {
    message += chalk4.dim(" (Use arrow keys)");
  ...
  const allChoices = choices.map((choice2, index) => {
    ...
    if (choice2.disabled) {
      ...
      return chalk4.dim(`- ${line} ${disabledLabel}`);
    }
    if (index === cursorPosition) {
      return chalk4.cyan(`${import_figures2.default.pointer} ${line}`);
    }
    return `  ${line}`;
  }).join("\n");
  ...
  if (status === "done") {
    return `${prefix} ${message} ${chalk4.cyan(choice.name || choice.value)}`;
  }
...
var log_default = (message, status = "default") => {
  if (status === "default")
    console.log("\n ", chalk5.hex("#000").bgGray(message));
  if (status === "info")
    console.log(chalk5.bgHex("#6366f1").white(" Info "), message);
  if (status === "success")
    console.log("\n ", chalk5.hex("#8EEDB3")(message));
  if (status === "important")
    console.log("\n ", chalk5.bgYellow.black(message));
  if (status === "error")
    console.error("\n ", chalk5.red(message));
};
...
import ora from "ora";
var loader_default = (text) => ora({ text, spinner: "dots2", discardStdin: false, color: "cyan" });

// src/shell/cloneRepos.ts
import shell from "shelljs";
var cloneRepos = (Repos, Dir) => new Promise((fulfill, reject) => {
...
var cloneRepos_default = cloneRepos;

// src/utilities/colorize.ts
import chalk6 from "chalk";
var colorize_default = (message, status = "default") => {
  if (status === "default")
    return chalk6.hex("#000").bgGray(message);
  if (status === "info")
    return `${chalk6.bgHex("#6366f1").white(" Info ")} ${message}`;
  if (status === "success")
    return chalk6.hex("#8EEDB3").bold(message);
  if (status === "important")
    return chalk6.bgYellow.black(message);
  if (status === "error")
    return chalk6.red(message);
  return "";
};

// src/actions/newToSave.ts
var exec = util.promisify(nodeExec);
var fsPromises = fs.promises;
async function bootUpSave() {
  const starterRepos = [
  ...
  try {
    ...
    if (!fs.existsSync(folderName)) {
      log_default("Creating Save directory...", "info");
      await fsPromises.mkdir(folderName);
    }
    const cloneLoader = loader_default(colorize_default("", "info")).start();
    await cloneRepos_default(
      starterRepos,
      folderName
    );
    ...
    console.error(chalk7.red(JSON.stringify(error)));
  }
}

// src/index.ts
import chalk8 from "chalk";
var figlet = __require("figlet");
(async () => {
  console.log(
    "\n",
    chalk8.cyan(
  ...
  const initialAnswer = await esm_default({
    message: colorize_default("What do you want to do", "info"),
    choices: [
      {
        name: "I'm new to save! (bootup apps for you)",
        value: "new",
        description: "?? This will clone api, dashboard and ussd for you and run them on Docker"
      },
      {
        name: "Deploy",
        value: "deploy",
